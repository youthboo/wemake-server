...
func SetupRoutes(db *sqlx.DB, cfg *config.Config) *fiber.App {
	...
	orderService := service.NewOrderService(db, orderRepo, walletRepo, transactionRepo)
	...
	showcases.Delete("/:showcase_id/images/:image_id", showcaseHandler.DeleteImage)
	showcases.Patch("/:showcase_id/images/:image_id", showcaseHandler.PatchImage)
	showcases.Get("/:showcase_id/specs", showcaseHandler.GetSpecs)
	showcases.Put("/:showcase_id/specs", showcaseHandler.BulkReplaceSpecs)
